...
void* mm_malloc (size_t size) {
  ...
  if(blockSize < reqSize + MIN_BLOCK_SIZE) {
    ...
    // Tag next block with TAG_PRECEDING_USED
    ptrNextBlock = (BlockInfo*)POINTER_ADD(ptrFreeBlock, blockSize);
    ptrNextBlock->sizeAndTags = ptrNextBlock->sizeAndTags | TAG_PRECEDING_USED;

    // Remove block from free list
    removeFreeBlock(ptrFreeBlock);
    //mm_check();
    //puts("===== end of mm_malloc=====\n");
    // Return the block
    ...
    removeFreeBlock(ptrFreeBlock);
    // Divide memory to 2 aligned blocks
    ptrFreeBlock->sizeAndTags = reqSize | TAG_USED | precedingBlockUseTag;
    ptrNextBlock = (BlockInfo*)POINTER_ADD(ptrFreeBlock, reqSize);
    // Tag the free block with TAG_PRECEDING_USED
    ptrNextBlock->sizeAndTags = (blockSize - reqSize) | TAG_PRECEDING_USED;
    // Insert the boundary tag at the end of the new block
    size_t* newBlockSize = (size_t*)POINTER_ADD(ptrNextBlock, SIZE(ptrNextBlock->sizeAndTags) - WORD_SIZE);
    *newBlockSize = (blockSize - reqSize) | TAG_PRECEDING_USED;
    // Insert free block in free list
...
void mm_free (void *ptr) {
  ...
  // Add a boundary tag
  size_t * boundarySize = (size_t*)POINTER_ADD(blockInfo, payloadSize - WORD_SIZE);
  *boundarySize = payloadSize;

  // Untag the following block
...
int mm_free_blocks_free() {
  BlockInfo* freeBlock;
  freeBlock = FREE_LIST_HEAD;
  while (freeBlock != NULL){
    if (((freeBlock->sizeAndTags & TAG_USED) != 0) && SIZE(freeBlock->sizeAndTags) != 0) {
      return 1;
    } else {
      freeBlock = freeBlock->next;
    }
...
void * mm_valid_free_blocks() {
  BlockInfo* freeBlock;
  freeBlock = FREE_LIST_HEAD;
  while (freeBlock != NULL) {
    size_t blockSize = SIZE(freeBlock->sizeAndTags);
    size_t* boundary_tag = (size_t*)POINTER_ADD(freeBlock, blockSize - WORD_SIZE);
    if (freeBlock->sizeAndTags != *boundary_tag) {
      return freeBlock;
    }

    freeBlock = (BlockInfo*)POINTER_ADD(freeBlock, blockSize);
  }
  return NULL;
}
...
int mm_check() {
  ...
  examine_heap();
  //Is every block free?
  if(mm_free_blocks_free()) {
    printf("A free block is not free!\n");
    return 1;
  }
  // Is there any continoues free block? (escaped coelesing)
  //if(mm_not_coalesced()) {
  //  printf("Not all blocks are coealesced");
  //  return 2;
  //}
  // Is all free blocks in the free list?
  // Are they valid free blocks in the free list?
  block = (BlockInfo*) mm_valid_free_blocks();
  if(block != NULL) {
    //printf(block->sizeAndTags, "block->sizeAndTags %d");
    //size_t* btag = (size_t*) POINTER_ADD(block, SIZE(block->sizeAndTags) - WORD_SIZE);
    //printf(*btag, "btag %d");
    return 3;
  }
  // Does any block overlap?
  // 
  return 0;
}
